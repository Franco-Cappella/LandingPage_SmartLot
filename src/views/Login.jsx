...
export default function Login() {
  ...
  useGSAP(() => {
    ...
    mm.add("(prefers-reduced-motion: reduce)", () => {
      gsap.set([".login-card", ".login-fade"], { opacity: 1, y: 0 });
    });
  }, { scope: container });
  ...
  return (
    <div ref={container} className="min-h-screen bg-[#F7F4EF] relative overflow-hidden flex items-center justify-center px-4 py-12">
      ...
      <div className="login-card w-full max-w-md glass-card p-10 md:p-12 relative z-10">
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="login-fade text-3xl md:text-4xl font-extrabold text-[#0E121B] text-center" style={{ fontFamily: 'var(--font-display)' }}>
            Iniciar Sesión
          </h1>
          <p className="login-fade text-[#5A6B8A] mt-2 text-center">
            Ingresá a tu panel de gestión
          </p>
        </div>

        <form className="flex flex-col gap-5" onSubmit={(e) => e.preventDefault()}>
          <div className="login-fade flex flex-col gap-1.5">
          ...
          <button
            type="submit"
            className="login-fade w-full mt-2 px-8 py-4 bg-[#2A5CBF] text-white rounded-[16px] font-bold text-lg hover:bg-[#0C1E3F] active:scale-[0.97] transition-all duration-300 shadow-lg shadow-[#2A5CBF]/20 hover:shadow-[#0C1E3F]/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2A5CBF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FDFCF9]"
            style={{ fontFamily: 'var(--font-body)' }}
          >
            Ingresar
          </button>
        ...
        <p className="login-fade mt-8 text-center text-sm text-[#5A6B8A]">
          ¿No tenés cuenta?{' '}
          <Link to="/register" className="text-[#2A5CBF] font-semibold hover:text-[#0C1E3F] transition-colors duration-300 focus-visible:outline-none focus-visible:underline">
            Registrarse
          </Link>
        </p>
      </div>
